...
export async function POST(request: Request) {
  ...
  try {
    ...
    const senderPhone = '+21622315295'; // Replace this with your number or retrieve dynamically
    const smsMessage = `Request successfully created with ID ${(result as OkPacket).insertId}.`;
    await sendSms(senderPhone, smsMessage);
...
async function sendSms(to: string, body: string) {
  ...
  if (!response.ok) {
    ...
    console.error('Twilio SMS Error:', errorData); // Log the error data
...
export async function PATCH(request: Request) {
  ...
  try {
    if (action === 'accept') {
      ...
      // Retrieve the receiver's phone number (assuming it's stored in the requests table)
      const [requestData] = await pool.query('SELECT id_profile_receiver FROM requests WHERE id_request = ?', [id]);
      const receiverId = requestData[0]?.id_profile_receiver;

      // Fetch the receiver's phone number from the profiles table
      const [receiverProfile] = await pool.query('SELECT phone_number FROM profiles WHERE id_profile = ?', [receiverId]);
      const receiverPhone = receiverProfile[0]?.phone_number;

      if (receiverPhone) {
        const smsMessage = `Your request with ID ${id} has been accepted.`;
